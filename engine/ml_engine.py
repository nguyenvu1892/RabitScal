...
# Scalping Fitness Weights — RabitScal v5.0 BALANCED SCALPING
# Triết lý: Volume là phương tiện, lợi nhuận mới là mục đích. Vô dụng nếu cày lỗ!
SCALP_WR_WEIGHT     = 60.0    # WR đãng giá hơn cược cày vô định hướng
SCALP_PF_WEIGHT     = 15.0    # Tăng PF weight: đảm bảo có lợi nhuận ròng
SCALP_COUNT_WEIGHT  = 0.3     # Vừa đủ: mỗi lệnh = 0.3 điểm
# Volume Bonus v5.0: CHỈ thưởng khi đầy đủ cả 3 điều kiện — không được cày lỗ!
VOL_BONUS_100  = 2.0   # ×2.0 nếu trades≥100 AND PF≥1.2 AND DD≤20%
VOL_BONUS_200  = 2.0   # giữ 2.0 (không cần đánh giá thêm cho vỡt trản)
# Formula v5.0:
#   base  = WR×60 + PF×15 + trades×0.3 - DD_penalty×60
#   score = base × 2.0  IF (trades≥100 AND PF≥1.2 AND DD≤20%)
#         = base × 1.0  OTHERWISE
DEFAULT_MIN_TRADES_SCALP = 30   # Scalper ≥30 lệnh — đủ thống kê, đủ tần suất
...
ASSET_CLASS_CONFIG: dict[str, dict] = {
    ...
    "METAL": {
        ...
        "min_trades":           50,
        "max_dd_limit":         0.40,
        # Exness Standard XAUUSD: spread ~18-25 points (biến động bình thường)
        # pip_value = $1/unit → spread_cost = 20 ticks = $0.20 rờng thường
        "spread_cost":          20.0,  # ticks (tương đương $0.20/micro-lot)
        "commission_per_lot":   0.0,   # Standard: không commission, chỉ spread
    ...
    "BTC": {
        ...
        "min_trades":           50,
        "max_dd_limit":         0.40,
        # Exness Standard BTCUSDm: spread ~120-200 USD biến động mạnh, ETH ~15-30 USD
        "spread_cost":          150.0, # ticks BTC ≈ $150 khi viết lệnh (thực tế cao hơn khi news)
        "commission_per_lot":   0.0,   # Standard: không commission
    ...
    # ── FOREX-MAJOR: EUR, GBP — Scalping $200 profile (5-10 lệnh/ngày)
    "FOREX-MAJOR": {
        "_symbols":             ["EUR", "GBP"],
        "atr_sl_multiplier":    (0.3,  2.0),   # SL chặt hơn cho scalping
        "atr_lot_multiplier":   (0.01, 0.05),
        "atr_fvg_buffer":       (0.3,  1.5),
        "vsa_volume_ratio":     (1.0,  2.5),   # Nới rộng để bắt nhiều lệnh
        "vsa_neighbor_ratio":   (0.9,  1.8),
        "vsa_min_score":        (0.15, 0.5),
        "pinbar_wick_ratio":    (0.40, 0.70),
        "pinbar_body_ratio":    (0.10, 0.45),
        "composite_score_gate": (0.05, 0.40),  # Hạ mạnh — scalping cần giọng sung
        "min_trades":           1050,           # ~1500/năm × 70% IS
        "max_dd_limit":         0.28,           # 28% — buffer dưới 30% Sếp Vũ OK
        "spread_cost":          0.00015,
        "pip_value":            1.0,
        "label":                "FOREX-MAJOR",
    },
    # ── FOREX: JPY, AUD, CHF — Scalping $200 profile
    "FOREX": {
        "_symbols":             ["JPY", "AUD", "CHF", "NZD", "CAD"],
        "atr_sl_multiplier":    (0.3,  2.0),
        "atr_lot_multiplier":   (0.01, 0.05),
        "atr_fvg_buffer":       (0.3,  1.5),
        "vsa_volume_ratio":     (1.0,  3.0),
        "vsa_neighbor_ratio":   (0.9,  2.0),
        "vsa_min_score":        (0.2,  0.6),
        "pinbar_wick_ratio":    (0.45, 0.75),
        "pinbar_body_ratio":    (0.10, 0.40),
        "composite_score_gate": (0.05, 0.45),  # Nới rộng cho scalping
        "min_trades":           1050,           # ~1500/năm × 70% IS
        "max_dd_limit":         0.28,
        "spread_cost":          0.00015,
        "pip_value":            1.0,
    ...
    "CRYPTO": {
        ...
        "min_trades":           10,
        "max_dd_limit":         0.40,
        # Exness Standard SOL/BNB: spread ~$0.05-0.30 tùy thời điểm
        # Standard account: SOL ~$0.10-0.20 spread per micro-lot
        "spread_cost":          8.0,   # ticks alt-crypto ≈ $0.08-0.20
        "commission_per_lot":   0.0,   # Standard: không commission
...
def run_backtest_fast(
    ...
    *,
    ...
    # V12.0: RR_RATIO là Optuna param — AI tự tìm RR tối ưu (không ép >= 1.1)
    RR_RATIO       = params.get("rr_ratio", 1.5)   # fallback = 1.5 nếu chưa có key
...
def _build_objective(
    ...
    def objective(trial: optuna.Trial) -> float:
        # ── V12.0: Suggest theo asset class search space ───────────────────
        # Triết lý: AI TỰ HỌC mọi param — không ép WR, không ép RR.
        params = {
            ...
            # AI TỰ QUYẼT ĐỊNH R/R tối ưu — không fi x cứng bất kỳ giá trị nào.
            # Search space rộng: 0.3 → 5.0 (log=True — sample đều log-scale)
            # TPE Multivariate tự tìm vùng tối ưu dựa trên fitness feedback
            "rr_ratio":             trial.suggest_float("rr_ratio", 0.3, 5.0, log=True),
            # V12.0 NEW: EQL/EQH weight — AI học mức độ "né" vùng thanh khoản
            "eql_weight":           trial.suggest_float("eql_weight", 0.0, 3.0),
            "eqh_weight":           trial.suggest_float("eqh_weight", 0.0, 3.0),
        }
        ...
        # ── V12.0: Net_EV-Centric Fitness ─────────────────────────────────
        # Triết lý Sếp Vũ: "Mục tiêu duy nhất để sinh tồn là Tiền (Lãi ròng)"
        # WR có thể 20%, RR có thể 0.5 — miễn Net_EV dương và DD an toàn.
        n_trades  = res["trade_count"]
        gross_p   = res.get("gross_profit",  0.0)
        gross_l   = res.get("gross_loss",    0.0)
        dd        = res["max_drawdown"]
        wr        = res["winrate"]
        pf        = res["profit_factor"]

        # Net_EV = Gross Profit - Gross Loss (đã khấu trừ spread trong backtest)
        net_ev = gross_p - gross_l

        # ── ĐIỀU KIỆN SINH TỒN DUY NHẤT ─────────────────────────────────────
        # Gate 1: Net_EV phải dương (không sinh lời → prune)
        if net_ev <= 0:
            raise optuna.exceptions.TrialPruned()
        # Gate 2: Max Drawdown trong ngưỡng an toàn đã cấu hình
        # (đã được kiểm tra qua res["pruned"] ở trên — double-check)
        # KHÔNG có gate WR, KHÔNG có gate RR, KHÔNG có gate PF.

        # ── Scalping $200 Fitness — Thưởng mạnh tần suất (Sếp Vũ V17.0) ────
        # Formula: Net_EV × (1+ln(trades/1000)) × (1-DD×0.3) × OOS_stability
        # Giải thích:
        #   freq_bonus: 1000L→+0, 1500L→+0.41, 2000L→+0.69, 3000L→+1.10
        #   dd_penalty: 0.3 (nhẹ hơn 0.4 cũ — ưu tiên tần suất hơn DD thấp)
        import math as _math
        freq_bonus = 1.0 + _math.log(max(n_trades, 1) / 1000.0) if n_trades >= 1000 else 0.5
        dd_ratio   = dd / max(max_dd_limit, 1e-8)
        net_ev_score = net_ev * freq_bonus * (1.0 - dd_ratio * 0.3)

        logger.debug(
            f"[Trial {trial.number}] [{sp.get('label','?')}] "
            f"WR={wr:.1%} PF={pf:.2f} "
            f"RR={params['rr_ratio']:.2f} DD={dd:.1%} "
            f"net_ev={net_ev:.4f} trades={n_trades} "
            f"freq_bonus={freq_bonus:.2f} score={net_ev_score:.4f}"
        )
...
def objective(trial: optuna.Trial) -> float:
    """
    V12.0 — Net_EV-Centric Fitness Function.
    Triết lý: AI tự học mọi thứ, tiền là thước đo DUY NHẤT.

    Score = Net_EV × (1 - DD_ratio × 0.4) + trade_bonus

    Điều kiện sinh tồn (chỉ 2):
        1. Net_EV > 0    (bắt buộc sinh lời)
        2. MaxDD < max_dd_limit  (bắt buộc an toàn)
    KHÔNG gate WR. KHÔNG gate RR. KHÔNG gate PF.
    ...
    # ── V12.0: Suggest tham số — AI tự do khám phá ────────────────────────
    params = {
        "atr_sl_multiplier":    trial.suggest_float("atr_sl_multiplier",    *sp["atr_sl_multiplier"]),
        "atr_lot_multiplier":   trial.suggest_float("atr_lot_multiplier",   *sp["atr_lot_multiplier"]),
        "atr_fvg_buffer":       trial.suggest_float("atr_fvg_buffer",       *sp["atr_fvg_buffer"]),
        ...
        "pinbar_body_ratio":    trial.suggest_float("pinbar_body_ratio",    *sp["pinbar_body_ratio"]),
        "composite_score_gate": trial.suggest_float("composite_score_gate", *sp["composite_score_gate"]),
        # V12.0 NEW: RR Optuna param — AI tự tìm RR tối ưu (không ép >= 1.1)
        "rr_ratio":             trial.suggest_float("rr_ratio", 0.3, 5.0, log=True),
        # V12.0 NEW: EQL/EQH weight — AI tự học mức độ né vùng thanh khoản
        "eql_weight":           trial.suggest_float("eql_weight", 0.0, 3.0),
        "eqh_weight":           trial.suggest_float("eqh_weight", 0.0, 3.0),
    }
    ...
    # ── Điều kiện sinh tồn (2 gates duy nhất — không có gate WR/RR) ───────
    if result.trade_count < min_trades:
        raise optuna.exceptions.TrialPruned()
    if result.max_drawdown >= max_dd_limit:
        raise optuna.exceptions.TrialPruned()

    net_ev = result.gross_profit - result.gross_loss
    if net_ev <= 0:
        raise optuna.exceptions.TrialPruned()   # Không sinh lời → triệt tiêu ngay

    # ── V12.0 Net_EV Score ────────────────────────────────────────────────
    dd_ratio      = result.max_drawdown / max(max_dd_limit, 1e-8)
    trade_bonus   = min(result.trade_count / 500.0, 1.0) * 0.05 * abs(net_ev)
    net_ev_score  = net_ev * (1.0 - dd_ratio * 0.4) + trade_bonus

    logger.debug(
        f"[Trial {trial.number}] [{sp.get('label','?')}] "
        f"WR={result.winrate:.1%} PF={result.profit_factor:.2f} "
        f"RR={params['rr_ratio']:.2f} DD={result.max_drawdown:.1%} "
        f"net_ev={net_ev:.4f} trades={result.trade_count} "
        f"score={net_ev_score:.4f}"
    )
...
def _shadow_deploy(
    ...
    *,
    ...
    # Dùng IS PF (best_metrics) làm baseline — tuyệt đối KHÔNG dùng active_config
    # vì active_config có thể chứa PF=999 từ run cũ (USTEC/US500 overfit) → OOS bar = 649!
    is_pf = max(float(best_metrics.profit_factor), 0.01)   # IS profit_factor, floor=0.01

    # ── Walk-forward OOS validation ────────────────────────────────────────
    logger.info(
        f"[ShadowDeploy] Running walk-forward OOS validate | "
        f"oos_hours={oos_hours} | is_pf={is_pf:.3f} | promote_thr={promote_thr}"
    )
    wf_result = _run_walk_forward_oos(
        full_data, best_params, oos_hours, is_pf, promote_thr, logger=logger
    )
...
def main() -> None:
    ...
    # Map deploy_status: promoted → "live", retired → "retired", shadow → "shadow"
    _ds_raw = result.deploy_status or ""
    _ds_out = ("live"    if _ds_raw == "promoted" else
               "shadow"  if _ds_raw == "shadow"   else
               "retired" if _ds_raw == "retired"  else _ds_raw)
    # avg_rr = PF × (1−WR)/WR — thước đo chất lượng lệnh
    _rr = (result.best_pf * (1 - result.best_winrate) / result.best_winrate
           if result.best_winrate > 0 else 0.0)
