#!/usr/bin/env python3
"""
pipeline_ev05c.py — Ev05c: Super Portfolio (Ev04 engine × Super 6)
===================================================================
Kế thừa CHÍNH XÁC Ev04 FVG engine:
  signal = feats @ weights  (dot-product linear classifier, weights ∈ optuna)
  TP     = Opposing FVG mid (Split Ticket 2-leg)
  SL     = ± sl_mult × ATR

Nâng cấp so với Ev04:
  1. Multi-symbol: Super 6 × 3 TF = 18 studies (queue tuần tự)
  2. H1 Trend Filter (Option B): feats[:, 23] = trend_ema_h1 → block ngược trend
  3. Margin-safe lot sizing: calc_lot_size_with_margin() (3% risk = $600)
  4. Global Risk Gate: tối đa 3 trade đồng thời trên toàn portfolio
  5. Symbol-aware spread/swap từ SYMBOL_PROPS

Architecture đã được chứng minh trong Ev04 (EURUSDm). Ev05c scale lên.
"""
...
from core.symbol_registry import (
    SYMBOL_PROPS, SUPER_6_SYMBOLS,
    calc_lot_size_with_margin, calc_margin,
)
from core.feature_engine import build_feature_matrix

logger = logging.getLogger("EV05C")
...
RISK_PER_TRADE  = 0.03          # 3% = $600/trade (Ev05c: tăng từ 1% Ev04)
MAX_DD_LIMIT    = 0.28
OOS_DD_GATE     = 0.25
MAX_CONCURRENT  = 3             # Global Risk Gate
MAX_MARGIN_PCT  = 0.30
FEAT_H1_TREND   = 23            # feats[:, 23] = trend_ema_h1 (-1/0/+1)

# Features frozen tại 0 (không tham gia dot-product), kế thừa từ Ev04
...
# TF search space (kế thừa Ev04, điều chỉnh cho multi-symbol)
TF_SS = {
...
# ─── FVG Zone Builder (copy từ feature_engine) ────────────────────────────────

def _build_fvg_zones(o, h, l, c, atr, lookback=100):
    """Tính FVG zones từ raw OHLCV — kế thừa từ Ev04/feature_engine."""
    from core.feature_engine import _nearest_opposing_fvg_zones
    return _nearest_opposing_fvg_zones(o, h, l, c, atr, lookback=lookback)


# ─── Core Backtest (kế thừa run_backtest_ev04 + H1 Filter) ─────────────────────

def run_backtest_ev05c(
    ...
    h1_filter:  bool   = True,
    h1_inside_bar: np.ndarray | None = None,  # NEW: force-close khi InsideBar
) -> dict | None:
    """
    Split Ticket backtest với H1 Trend Filter.

    Signal = feats @ weights (dot product) — giống Ev04.
    H1 Filter: feats[:, 23] = trend_ema_h1:
      +1 (bull) → chỉ LONG, block SHORT
      -1 (bear) → chỉ SHORT, block LONG
       0 (sideways) → cho phép cả hai

    Lot sizing dùng calc_lot_size_with_margin() với margin check.
    """
    N = len(features)
    if N < 500:
        return None

    # ── Signal generation (Ev04 core) ──────────────────────────────────────────
    score = features.astype("float32") @ weights
    h1_trend = features[:, FEAT_H1_TREND]  # -1/0/+1

    if h1_filter:
        # Block lệnh ngược trend H1
        ls = (score >  threshold) & (h1_trend >= 0)   # LONG: H1 bull hoặc sideways
        ss = (score < -threshold) & (h1_trend <= 0)   # SHORT: H1 bear hoặc sideways
    else:
        ls = score >  threshold
        ss = score < -threshold

    ls[:200] = False; ss[:200] = False  # warm-up period
    ...
    for i in range(N):
        ...
        if in_a:
            ...
            if hit_tp_a:
                ...
                if in_b:   # Leg B BE trail
                    sl_b = ep_b + slip_i * d_b
            elif hit_sl_a:
        ...
        if not in_a and not in_b:
            ...
            # Lot sizing với margin check (Ev05c upgrade)
            entry_price = op
            lot_total = calc_lot_size_with_margin(
...
def _ev05c_worker(tid, params, symbol, tf,
                  fn, fs, fd, rn, rs, rd, mn, ms, md,
                  zn_names, zn_shapes, zn_dtypes,
                  ibn, ibs, ibd,  # h1_inside_bar SHM
                  spread):
    ...
    try:
        ...
        h1_ib,  _s4 = att(ibn, ibs, ibd)   # h1_inside_bar
...
def train_study_ev05c(
    ...
    study_name = f"ev05c_{sym_plain}_{tf.lower()}"
    ...
    ibn, ibs, ibd = alloc(h1_ib)  # h1_inside_bar SHM
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as exe:
            while done < remaining:
                ...
                for _ in range(batch):
                    t = study.ask()
                    # Weights: chỉ suggest ACTIVE_FEATURES (53 dims)
                    # FROZEN_FEATURES = 0.0 (không suggest — tránh low==high bug)
                ...
                futures = {
                    exe.submit(
                        _ev05c_worker, t.number, p, symbol, tf,
                        fn, fs, fd, rn, rs, rd, mn, ms, md,
                        zn_names, zn_shapes, zn_dtypes,
                        ibn, ibs, ibd,   # h1_inside_bar SHM — Bug #3 fix
                        spread,
                    ): t
                    for t, p in asked
                }
...
def run_ev05c_queue(
    ...
    """Queue tuần tự: symbol × TF. Sau khi xong gọi portfolio_report."""
    if tfs is None:
        tfs = ["M1", "M5", "M15"]
    ...
    print(f"\n{'='*65}")
    print(f"  EV05c SUPER PORTFOLIO — {len(symbols)} symbols × {len(tfs)} TF")
    print(f"  Signal: feats@weights | H1 Filter ON | 3% risk | Gate=3")
    ...
    for i, sym in enumerate(symbols):
        for j, tf in enumerate(tfs):
            ...
            try:
                train_study_ev05c(sym, tf, data_dir, n_trials, n_workers, spread)
            except Exception as e:
    ...
    print(f"\n{'='*65}")
    print(f"  ✅ EV05c HOÀN TẤT — {total} studies × {n_trials} trials!")
    print(f"  Generating Portfolio Fact Sheet...")
    print(f"{'='*65}")

    # Auto-generate Fact Sheet
    try:
        from engine.portfolio_report import generate_fact_sheet
        generate_fact_sheet(
            output_path="logs/ev05c_fact_sheet.md",
            data_dir=data_dir,
        )
    except Exception as e:
