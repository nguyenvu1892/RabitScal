...
DATA_DIR  = ROOT / "data"
LOGS_DIR  = ROOT / "logs"
# ── Python interpreter: ưu tiên venv local (có numpy/optuna), fallback sys.exec
_VENV_PY  = ROOT / "venv" / "bin" / "python"
PYTHON    = str(_VENV_PY) if _VENV_PY.exists() else sys.executable
ML_SCRIPT = str(ROOT / "ml_model.py")

# ── Parallel config (Dual Xeon 56 threads) ────────────────────────────────
PARALLEL_JOBS    = 7    # file chạy cùng lúc
WORKERS_PER_JOB  = 8    # workers mỗi file  →  7 × 8 = 56 luồng (khớp Xeon)

# ── Lưới 1 ────────────────────────────────────────────────────────────────
SHALLOW_TRIALS          = 100
SHALLOW_MIN_TRADES      = 30   # Scalper ≥30 lệnh: đủ thống kê, đủ tần suất
CRYPTO_MIN_TRADES       = 40   # CRYPTO/BTC: cần thưa hơn vì spread cao
SHALLOW_TIMEOUT         = 600
SHALLOW_GATE_WR         = 0.55

# ── Lưới 2 ────────────────────────────────────────────────────────────────
DEEP_TRIALS      = 1000
DEEP_TIMEOUT     = 3600   # 60 phút/file (tăng vì 1000 trials)

# ── Nghiệm thu ────────────────────────────────────────────────────────────
CONFIRM_WR = 0.55   # Hạ từ 0.65→0.55: volume uu tiên, WR vừa đủ là chạy
...
class ScanResult:
    ...
    winrate:       float = 0.0
    pf:            float = 0.0
    rr:            float = 0.0   # avg_win/avg_loss = PF×(1-WR)/WR
    max_dd:        float = 0.0
    trades:        int   = 0
...
def filter_luoi_1(results: list[ScanResult]) -> tuple[list[ScanResult], list[ScanResult]]:
    ...
    for r in results:
        ...
        # Per-class min_trades: CRYPTO khắt hơn (30) vì dễ ít lệnh, dễ may mắn
        class_min = CRYPTO_MIN_TRADES if r.asset_class in ("CRYPTO", "BTC") else SHALLOW_MIN_TRADES
        fails = []
        if r.winrate <= SHALLOW_GATE_WR:
            fails.append(f"WR={r.winrate:.1%} ≤ {SHALLOW_GATE_WR:.0%}")
        if r.trades < class_min:
            fails.append(f"rổ={r.trades} < {class_min}")
        if fails:
            r.status = "filtered"
...
def write_report(
    ...
    lines = [
        ...
        f"",
        f"| # | Symbol | TF | Asset Class | Score | Winrate | **Avg RR** | PF | Max DD | Rổ | OOS |",
        f"|---|--------|----|-------------|------:|:-------:|:----------:|:--:|:------:|:--:|:---:|",
    ]
    medals = {1:"🥇", 2:"🥈", 3:"🥉"}
    for i, r in enumerate(confirmed, 1):
        rr_badge = (f"✅ **{r.rr:.2f}**" if r.rr >= 1.1 else f"⚠️ {r.rr:.2f}")
        lines.append(
            f"| {medals.get(i, str(i))} | **{r.symbol}** | {r.tf} | {r.asset_class}"
            f" | **{r.score:.2f}** | {_wr_badge(r.winrate)}"
            f" | {rr_badge} | {r.pf:.2f} | {_dd_badge(r.max_dd)} | {r.trades} | {r.oos_status} |"
        )
