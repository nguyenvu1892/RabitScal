...
## 🔄 Task 2.1: `strategy_engine.py` — Implementation (PENDING TechLead snippet review)

**Date:** 2026-03-05 23:10 UTC+7 | **Branch:** `task-2.1-strategy-engine`
...
- **FVG Pool (Violated vs Mitigated):** Phân biệt 2 lý do xóa FVG: (1) *Violated* = close xuyên boundary → cấu trúc gãy; (2) *Mitigated* = đã trigger signal → tránh double entry. TTL=48 nến M15, max_size=20 (FIFO).
- **Pinbar Validation:** wick_ratio ≥ 60%, body_ratio ≤ 35%. BẮT BUỘC wick phải là phần chạm FVG (low ≤ fvg.top cho BUY, high ≥ fvg.bottom cho SELL).
- **VSA 2-Layer Filter:** Layer 1 = vol ≥ session_mean × 1.5x (baseline riêng London/NY/Global); Layer 2 = vol > prev_candle.vol × 1.2x (chống climax ảo nến liền kề).
